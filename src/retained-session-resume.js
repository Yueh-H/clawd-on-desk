...
} = require("./claude-desktop-session");
...
function getRetainedSessionResumeTarget(entry, options = {}) {
  ...
  if (entry.agentId === "claude-code") {
    ...
    return sessionId
      ? { canResume: true, type: "claude-auto", sessionId, url: null }
...
async function resumeRetainedSession(entry, options = {}) {
  ...
  try {
    ...
    if (target.type === "claude-auto") {
      const resolveDesktopSession = options.resolveClaudeDesktopSession || findClaudeDesktopSession;
      let desktopSession = null;
      try {
        desktopSession = await resolveDesktopSession(target.sessionId, {
          ...options,
          cwd: normalizeString(entry && entry.cwd),
        });
      } catch (_err) {}
      const desktopUrl = desktopSession
        ? buildClaudeDesktopSessionUrl(desktopSession.sessionId)
        : null;
      if (desktopUrl && typeof options.openClaudeDesktopSession === "function") {
        try {
          const opened = await options.openClaudeDesktopSession(desktopUrl, entry, desktopSession);
          if (opened !== false) {
            return { status: "ok", action: "open-claude-desktop-session" };
          }
        } catch (_err) {}
      }
