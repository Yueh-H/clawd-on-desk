...
test("Claude Desktop retained sessions reopen the exact existing app task", async () => {
  const localSessionId = "local_0689e98a-f9fe-415f-ae98-2bf3184f47bb";
  const urls = [];
  const result = await resumeRetainedSession(retained("claude-code", CLAUDE_ID), {
    resolveClaudeDesktopSession: async () => ({ sessionId: localSessionId }),
    openClaudeDesktopSession: async (url) => { urls.push(url); return true; },
    launchClaudeSession: async () => { throw new Error("must not launch Terminal"); },
  });

  assert.deepStrictEqual(result, { status: "ok", action: "open-claude-desktop-session" });
  assert.deepStrictEqual(urls, [`claude://claude.ai/epitaxy/${localSessionId}`]);
});

test("Claude CLI retained sessions launch --resume in the stored project directory", async () => {
  const launches = [];
  const result = await resumeRetainedSession(retained("claude-code", CLAUDE_ID), {
    osPlatform: "darwin",
    statSync: () => directoryStat(),
    resolveClaudeDesktopSession: async () => null,
...
test("Claude Desktop open failures fall back to the recoverable CLI session", async () => {
  const launches = [];
  const result = await resumeRetainedSession(retained("claude-code", CLAUDE_ID), {
    osPlatform: "darwin",
    statSync: () => directoryStat(),
    resolveClaudeDesktopSession: async () => ({
      sessionId: "local_0689e98a-f9fe-415f-ae98-2bf3184f47bb",
    }),
    openClaudeDesktopSession: async () => false,
    launchClaudeSession: async (...args) => {
      launches.push(args);
      return { ok: true, terminal: "osascript" };
    },
  ...
  assert.deepStrictEqual(result, { status: "ok", action: "launch-terminal", terminal: "osascript" });
  assert.deepStrictEqual(launches, [["resume", "/safe/project", CLAUDE_ID]]);
});
